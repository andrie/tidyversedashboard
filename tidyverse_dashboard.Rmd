...
largest_groups <- function(x, n = 5) {
  groups <- min_rank(desc(group_size(x))) <= n
  x[group_indices(x) %in% which(groups), ]
}
...
issue_progress_running %>%
  ...
  largest_groups(9) %>%
  ungroup() %>%
  mutate(maintainer = sub("<.*", "", maintainer)) %>%
  ggplot(aes(event, running_total, color = maintainer, group = type)) +
    geom_step() +
    labs(x = NULL, y = "Open Issues", color = "package maintaner") +
    facet_wrap(~type) +
